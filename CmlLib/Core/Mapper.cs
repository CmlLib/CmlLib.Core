...
public static class Mapper
{
    ...
    public static string Interpolation(string str, Dictionary<string, string?> dicts, bool handleEmpty)
    {
        str = argBracket.Replace(str, match =>
        {
            ...
            if (dicts.TryGetValue(key, out var value))
            {
                if (value == null)
                    value = "";
    ...
    private static string replaceByPos(string input, string replace, int startIndex, int length)
    {
        var sb = new StringBuilder(input);
        return replaceByPos(sb, replace, startIndex, length);
    }

    private static string replaceByPos(StringBuilder sb, string replace, int startIndex, int length)
    {
