...
public class MVersionMetadataSorter
{
    ...
    public MVersionMetadataSorter(MVersionSortOption option)
    {
        ...
        for (var i = 0; i < option.TypeOrder.Length; i++)
        {
    ...
    private int getTypePriority(MVersionType type)
    {
        ...
        if (typePriority.TryGetValue(type, out var p))
            return p;
    ...
    {
        var v1r = System.Version.TryParse(v1.Name, out var v1v);
        var v2r = System.Version.TryParse(v2.Name, out var v2v);
    ...
    private int compare(MVersionMetadata v1, MVersionMetadata v2)
    {
        ...
        {
            var result = compareProperty(v1, v2, propOption);
            if (result != 0)
                return result;
        }
