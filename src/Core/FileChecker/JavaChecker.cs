...
public class JavaChecker : IFileChecker
{
    ...
    public JavaChecker(HttpClient httpClient)
    {
        _httpClient = httpClient;
    }

    public JavaChecker(HttpClient httpClient, IJavaPathResolver javaPathResolver)
    {
        _httpClient = httpClient;
        ...
        IProgress<DownloadFileChangedEventArgs>? downloadProgress)
    {
        ...
        try
        {
            ...
            var response = await _httpClient.GetAsync(JavaManifestServer); // get all java versions
            var str = await response.Content.ReadAsStringAsync();
            using var jsonDocument = JsonDocument.Parse(str);
            
            var root = jsonDocument.RootElement;
            var javaVersions = root.GetPropertyOrNull(osName); // get os specific java version
    ...
    {
        var versionArr = job.GetPropertyOrNull(version)?.EnumerateArray();
        if (versionArr == null)
            return null;

        var firstManifest = versionArr.Value.FirstOrDefault();
        var manifestUrl = firstManifest.GetPropertyOrNull("manifest")?.GetPropertyOrNull("url")?.GetString();
    ...
    private async Task<JavaCheckResult> legacyJavaChecker()
    {
        ...
        MJava mJava = new MJava(_httpClient, legacyJavaPath);
